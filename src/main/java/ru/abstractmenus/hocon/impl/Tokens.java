...
final class Tokens {
    ...
    static private class Substitution extends Token {
        ...
        Substitution(ConfigOrigin origin, boolean optional, boolean relative, List<Token> expression) {
    ...
    static boolean getSubstitutionRelative(Token token) {
        if (token instanceof Substitution) {
            return ((Substitution) token).relative();
        } else {
            throw new ConfigException.BugOrBroken("tried to get substitution relative from "
                    + token);
        }
    }
    ...
    static Token newSubstitution(ConfigOrigin origin, boolean optional, boolean relative, List<Token> expression) {
        return new Substitution(origin, optional, relative, expression);
    }
